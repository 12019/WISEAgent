...
void smloader_connect_status_update_cb(susiaccess_agent_conf_body_t const * conf, susiaccess_agent_profile_body_t * profile, int status)
{
	if(g_SAManager)
	{
		if(g_SAManager->SAManager_UpdateConnectState_API)
			g_SAManager->SAManager_UpdateConnectState_API(conf, profile, status);
	}
}
...
void smloader_connect_status_update(susiaccess_agent_conf_body_t * config, susiaccess_agent_profile_body_t * profile, int status)
{
	...
	evtqueue_push(config, profile, status);
}
	...
	if(g_SAManager)
		{
			if(g_SAManager->SAManager_InternalSubscribe_API)
				g_SAManager->SAManager_InternalSubscribe_API(profile);
		}
}