...
void saclient_on_connect_cb(int rc)
{
	...
	if(saclient_send_agentinfo(AGENT_STATUS_ONLINE) == true)
	{
		...
		smloader_connect_status_update(g_config, g_profile, AGENT_STATUS_ONLINE);
	} else {
...
void saclient_on_lost_connect_cb(int rc)
{
	...
	SAClientLog(g_coreloghandle, Warning, "Lost connection: %s", MQTT_GetErrorString(rc));

	smloader_connect_status_update(g_config, g_profile, AGENT_STATUS_CONNECTION_FAILED);
...
void saclient_on_disconnect_cb(int rc)
{
	...
	smloader_connect_status_update(g_config, g_profile, AGENT_STATUS_OFFLINE);
...
int SACLIENT_API saclient_initialize(susiaccess_agent_conf_body_t * config, susiaccess_agent_profile_body_t * profile, void * loghandle)
{
	...
	smloader_callback_set(saclient_publish, saclient_subscribe, saclient_server_connect_config, saclient_disconnect);
...
int SACLIENT_API  saclient_subscribe(char const * topic, SACLIENT_MESSAGE_RECV_CALLBACK msg_recv_callback)
{
	...
	result = MQTT_Subscribe(g_mosq, (char *)topic, 1);
	if(result == mqtt_err_success)
	{
