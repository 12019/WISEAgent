...
void SAManager_CustMessageRecv(char* topic, susiaccess_packet_body_t *pkt, void *pRev1, void* pRev2)
{
	...
	if(topicentry != NULL)
	{
		char* pReqInfoPayload = pkg_parser_packet_print(pkt);
...
void SAMANAGER_API SAManager_InternalSubscribe(susiaccess_agent_profile_body_t * profile)
{
	/* Add Topic Callback*/
	char topicStr[128] = {0};
...
void SAMANAGER_API SAManager_UpdateConnectState(susiaccess_agent_conf_body_t const * conf, susiaccess_agent_profile_body_t * profile, int status)
{
	...
	if(g_pSALoader)
	{
		hlloader_handler_agent_status_update(g_pSALoader, &g_handlerList, conf, profile, status);
	}
}
