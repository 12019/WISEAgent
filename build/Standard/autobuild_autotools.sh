...
if [ -d "${SRC_ROOT}/.Release" ]; then
  rm -rf "${SRC_ROOT}/.Release"
fi
...
cp -f ${TEMP_RELEASE_DIR}/lib/*.so* ${CAGENT_OUTPUT_PATH}/module/
      
mv -f ${CAGENT_OUTPUT_PATH}/module/lib*.so* ${CAGENT_OUTPUT_PATH}/
   
cp -f ${TEMP_RELEASE_DIR}/bin/* ${CAGENT_OUTPUT_PATH}/
...
"${MAKESELF_TOOL_PATH}/makeself.sh" "${OUTPUT_PATH}/" "${PACKED_FILE_NAME}" "The Installer for SUSIAccess Agent" "./${UPDATER_SHELL_FILE}"

if [ ! -d "${PACKED_PATH}" ]; then
  mkdir "${PACKED_PATH}"
fi
mv "${PACKED_FILE_NAME}" "${PACKED_PATH}/"
cp -af "${DOC_PATH}/Install manual" "${PACKED_PATH}/"
