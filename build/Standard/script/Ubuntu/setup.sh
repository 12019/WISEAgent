...
CMA_SCRIPT="/etc/init/saagent.conf"
SAAGENT_START="start saagent"
SAAGENT_STOP="stop saagent"
SAAGENT_STATUS="status saagent"
SAAGENT_RESTART="restart saagent"
...
pre_setup_actions ()
{
	...
	stop sawatchdog   1>/dev/null 2>&1
	stop sawebsockify 1>/dev/null 2>&1
	stop saagent      1>/dev/null 2>&1
}
post_setup_actions ()
{
	echo "sending request to start AgentService..." 
	#start saagent 2>&1
	start sawebsockify 2>&1
	start sawatchdog   2>&1
}
check_linux_version ()
{
...
#	if [ $reconn == "disable" ]; then
		...
		if [ "$susiaccess_start_ans" == "y" ] ||[ "$susiaccess_start_ans" == "Y"  ] ||[ "$susiaccess_start_ans" == ""  ]; then
			local IsRunning=`$SAAGENT_STATUS 2>&1 | grep process`
			if [ -z $IsRunning ]; then
